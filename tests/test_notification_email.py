...
def test_deep_link_internal_project(monkeypatch):
    ...
    assert ne._deep_link("p1", Role.ESTIMATING_ENGINEER_MATERIALS.value) == f"{FRONTEND}/projects/p1"
...
def test_deep_link_nudge_goes_to_todos(monkeypatch):
    _patch_frontend(monkeypatch)
    assert ne._deep_link(None, Role.ESTIMATING_ENGINEER_MATERIALS.value, "nudge") == f"{FRONTEND}/todos"
    ...
    monkeypatch.setattr(ne, "get_supabase", lambda: _FakeSupabase({
        "profiles": [
            {"id": "pe1", "full_name": "Pat E", "email": "pat@g3.com",
             "role": "estimating_engineer_materials", "is_active": True},
            {"id": "off1", "full_name": "No Mail", "email": None,
             "role": "estimating_engineer_materials", "is_active": True},
            {"id": "gone", "full_name": "Inactive", "email": "x@g3.com",
             "role": "estimating_engineer_materials", "is_active": False},
        ],
...
def test_send_failure_is_isolated(monkeypatch):
    ...
    monkeypatch.setattr(ne, "get_supabase", lambda: _FakeSupabase({
        "profiles": [
            {"id": "a", "full_name": "A", "email": "a@g3.com", "role": "estimating_engineer_materials", "is_active": True},
            {"id": "b", "full_name": "B", "email": "b@g3.com", "role": "estimating_engineer_materials", "is_active": True},
        ],
        "projects": [],
    }))
